...
def clustering(tipoDist, numObj, minK, maxK, dataset, expDir, alg):
    ...
    """
    subprocess.check_output(['./clustering', tipoDist, numObj, minK, maxK, dataset, expDir])
...
def mocle(crossover, dataset, popIniDir, resultDir, truePartition):
    ...
    ---------
        ...
        truePartition : fake
            é inútil
    """
    subprocess.check_output(['./mocle', crossover, dataset, popIniDir, resultDir, truePartition])
