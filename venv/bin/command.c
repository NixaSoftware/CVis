...
{ printf ("Cluster 3.0, command-line version.\n");
  ...
  printf ("  -y number     Specifies the vertical dimension of the SOM grid\n"
          "                (default: 1)\n");
  printf ("  -minK number   Valor minimo de k para gerar cortes na arvore gerada com o hierarquico\n"
          "                (default: 2)\n");
  printf ("  -maxK number   Valor maximo de k para gerar cortes na arvore gerada com o hierarquico\n"
          "                (default: 2)\n");
  return;
}
...
"using the C Clustering Library version 1.30.\n"
"\n"
...
static void
HierarchicalPartition(char genemetric, char arraymetric, char method, char* jobname, int minK)//, int maxK)
{ FILE* outputfile;
  ...
  { //sprintf(filename, "%s.gtr", jobname);
    ...
    //else
    //{ 
    HierarchicalClusterPartition(jobname, genemetric, 0, method, minK);//, maxK);
    //  fclose(outputfile);
    //}
  }
  if (arraymetric)
  { //sprintf(filename, "%s.atr", jobname);
    ...
    //else
    //{ 
    HierarchicalClusterPartition(jobname, arraymetric, 1, method, minK);//, maxK);
    //  fclose(outputfile);
    //}
  }
...
{ int ifound;
  ...
  char* filename;
  char* errorfilename;
  int n = 1 + strlen(jobname) + strlen("-K") + strlen(".ext");
...
//  errorfilename = malloc(n*sizeof(char));
  filename = malloc(1000*sizeof(char));
  errorfilename = malloc(1000*sizeof(char));
  if (genemetric)
  { const char method = 'a';
    //sprintf (filename, "%s_K_G%d.kgg", jobname, k); //k
    sprintf (filename, "%s.clu", jobname); //k
    sprintf (errorfilename, "%s.err", jobname);
    outputfile = fopen(filename, "wt");
    errfile = fopen(errorfilename, "wt");
    if (!outputfile) printf ("Failed to open output file %s\n", filename);
    else
    { int* NodeMap = malloc(Rows*sizeof(int));
      ifound = GeneKCluster(k, r, method, genemetric, NodeMap, &error);
      SaveGeneKCluster(outputfile, k, NodeMap);
      fprintf(errfile, "%d\t%f", ifound, error);
      fclose(outputfile);
      fclose(errfile);
      free(NodeMap);
    }
  ...
  if (arraymetric)
  { const char method = 'a';
    //sprintf (filename, "%s_K_A%d.kag", jobname, k);  //k
    sprintf (filename, "%s.clu", jobname, k);
    outputfile = fopen(filename, "wt");
    if (!outputfile) printf ("Failed to open output file %s\n", filename);
    else
    { int* NodeMap = malloc(Columns*sizeof(int));
      ifound = ArrayKCluster(k, r, method, arraymetric, NodeMap, &error);
...
//// tirei quando fui executar o clustering
...
{ char* filename;
  ...
  int n = 1 + strlen(jobname) + strlen("-SOM") + strlen(".ext");
  ...
  filename = malloc(n*sizeof(char));
  sprintf (filename, "%s-SOM", jobname);
  if (genemetric) sprintf (strchr(filename,'\0'),"-G%d-%d",x,y);
  if (arraymetric) sprintf (strchr(filename,'\0'),"-A%d-%d",x,y);
...
{ int i = 1;
  ...
  { const char* const argument = argv[i];
    ...
    if(!strcmp(argument, "-cut"))//"-min"))
      { if (i==argc)
        { printf ("Error reading command line argument minK: parameter missing\n");
          return 0;
        }
        minK = readnumber(argv[i]);
        if (minK < 1)
        { printf ("Error reading command line argument mink: a positive integer is required\n");
          return 0;
        }
        i++;
        continue;
...
//          return 0;
//        }
//        maxK = readnumber(argv[i]);
//        if (maxK < 1)
//        { printf ("Error reading command line argument maxK: a positive integer is required\n");
//          return 0;
//        }
//        i++;
//        continue;
//      }
  ...
  { if (l) LogTransform();
    ...
    else HierarchicalPartition(genemetric, arraymetric, method, jobname, minK);//, maxK);//Hierarchical(genemetric, arraymetric, method, jobname);
    free(jobname);
  }
  Free();
  return 0;
