...
int HierarchicalClusterPartition(char* jobname, char metric, int transpose, char method, int minK)//, int maxK)
{ 
  FILE* outputfile;
  const int n = strlen(jobname) + strlen("xxxxxx.ext") + 1;
...
//  // grava as particoes com os varios valores de k
//  int *clusterid = malloc(nElements*sizeof(int));
//  int k;
//  for (k = minK; k <= maxK; k++)
//  {
//     cuttree(nElements, NodeElement, k, clusterid);
//     
//     sprintf(filename, "%sk%d.clu", jobname, k);
//     //printf("%s\n", filename);
...
//       {
//         //const int j = geneindex[a];
//         fprintf(outputfile, "%s\t%d\n", _geneuniqID[a], clusterid[a]);
//         //printf("%s\t%d\n", _geneuniqID[a], clusterid[a]);
//       }
//     }
//  }
...
//    fprintf (file, "%f\n", 1.0-NodeDistance[i]);
  ...
  // corta a arvore e grava a particao com k clusters
  int *clusterid = malloc(nElements*sizeof(int));
  cuttree(nElements, NodeElement, minK, clusterid);
  sprintf(filename, "%s.clu", jobname);
  outputfile = fopen(filename, "wt");
  if (!outputfile) printf ("Failed opening output file %s\n", filename);
  else
  {        
...
int GeneKCluster(int k, int nTrials, char method, char dist, int* NodeMap, double* error)
{ int ifound = 0;
  //double error;
...
int ArrayKCluster(int k, int nTrials, char method, char dist, int* NodeMap, double* error)
{ int ifound = 0;
  //double error;
  kcluster (k, _rows, _columns, _data, _mask,
