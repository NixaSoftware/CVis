	...
	unsigned int i; for (i = 0; i < size (); i++) {
		...
		if (i == 0) {
			int iobj; for (iobj = 0; iobj < NumberOfObjectives; iobj++) {
				ApproximateIdealPoint.ObjectiveValues [iobj] = 
                        (*this)[i]->ObjectiveValues [iobj];
				ApproximateNadirPoint.ObjectiveValues [iobj] = 
                        (*this)[i]->ObjectiveValues [iobj];
			}
		}
		else {
			int iobj; for (iobj = 0; iobj < NumberOfObjectives; iobj++) {
				if (Objectives [iobj].bActive) {
					if (Objectives [iobj].ObjectiveType == _Max) {
						if (ApproximateIdealPoint.ObjectiveValues [iobj] < 
                                     (*this)[i]->ObjectiveValues [iobj]) {
							ApproximateIdealPoint.ObjectiveValues [iobj] = 
                                   (*this)[i]->ObjectiveValues [iobj];
						}
						if (ApproximateNadirPoint.ObjectiveValues [iobj] > 
                                    (*this)[i]->ObjectiveValues [iobj])
							ApproximateNadirPoint.ObjectiveValues [iobj] =                                     
                                   (*this)[i]->ObjectiveValues [iobj];
					}
