...
#include "completelink.h" // class's header file

using std::string;

const int completelink::kDefault;

// class constructor
completelink::completelink(std::string datasetName, std::string fileResultName,
...
completelink::completelink(std::string datasetName, std::string fileResultName, t_similarities sim, std::string parameters)
{
	dataset = datasetName;
	fileResult = fileResultName;
    similarity = sim;
...
completelink::completelink(std::string parameters)
{
	dataset = "";
	fileResult = "";
    similarity = euclidean;
...
completelink::completelink(const completelink &a)
{
    dataset = a.dataset;
...
completelink & completelink::operator = (const completelink &a)
{
    dataset = a.dataset;
    fileResult = a.fileResult;
    similarity = a.similarity;
...
void completelink::changeDataset(std::string datasetName)
{
    dataset = datasetName;
}

void completelink::changeResultFile(std::string fileResultName)
{
    fileResult = fileResultName;
}

void completelink::changeSimilarity(t_similarities sim)
{
    similarity = sim;
}

void completelink::changeK(int value) // number of clusters
{
    k = value;
}

cpartition completelink::getPartition()
{
    return p;
}

//completelink::t_centroids kmeans::getCentroids()
//{
//    int nDimensions;
//    
//    if (p.nPatterns() == 0)
