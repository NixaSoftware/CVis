...
#include "averagelink.h" // class's header file

using std::string;

const int averagelink::kDefault;

// class constructor
averagelink::averagelink(std::string datasetName, std::string fileResultName, 
...
averagelink::averagelink(std::string datasetName, std::string fileResultName, t_similarities sim, std::string parameters)
{
	dataset = datasetName;
	fileResult = fileResultName;
    similarity = sim;
...
averagelink::averagelink(std::string parameters)
{
	dataset = "";
	fileResult = "";
    similarity = euclidean;
...
averagelink::averagelink(const averagelink &a)
{
    dataset = a.dataset;
...
averagelink & averagelink::operator= (const averagelink &a)
{
    dataset = a.dataset;
    fileResult = a.fileResult;
    similarity = a.similarity;
...
void averagelink::changeDataset(std::string datasetName)
{
    dataset = datasetName;
}

void averagelink::changeResultFile(std::string fileResultName)
{
    fileResult = fileResultName;
}

void averagelink::changeSimilarity(t_similarities sim)
{
    similarity = sim;
}

void averagelink::changeK(int value) // number of clusters
{
    k = value;
}

cpartition averagelink::getPartition()
{
    return p;
}

//averagelink::t_centroids kmeans::getCentroids()
//{
//    int nDimensions;
//    
//    if (p.nPatterns() == 0)
...
//  
//   
//   // soma os valores dos padres de cada cluster
...
//       }
//   }
//   // media dos valores dos padres (centroides)
//   for (c = clusterLabels.begin(); c != clusterLabels.end(); c++)
//   {
//       //std::cout << "cluster: " << *c << std::endl;
//       for (int i = 0; i < nDimensions; i++)
