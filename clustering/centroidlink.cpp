...
#include "centroidlink.h" // class's header file

using std::string;

const int centroidlink::kDefault;

// class constructor
centroidlink::centroidlink(std::string datasetName, std::string fileResultName,
...
centroidlink::centroidlink(std::string datasetName, std::string fileResultName, t_similarities sim, std::string parameters)
{
	dataset = datasetName;
	fileResult = fileResultName;
    similarity = sim;
...
centroidlink::centroidlink(std::string parameters)
{
	dataset = "";
	fileResult = "";
    similarity = euclidean;
...
centroidlink::centroidlink(const centroidlink &a)
{
    dataset = a.dataset;
...
centroidlink & centroidlink::operator = (const centroidlink &a)
{
    dataset = a.dataset;
    fileResult = a.fileResult;
    similarity = a.similarity;
...
void centroidlink::changeDataset(std::string datasetName)
{
    dataset = datasetName;
}

void centroidlink::changeResultFile(std::string fileResultName)
{
    fileResult = fileResultName;
}

void centroidlink::changeSimilarity(t_similarities sim)
{
    similarity = sim;
}

void centroidlink::changeK(int value) // number of clusters
{
    k = value;
}

cpartition centroidlink::getPartition()
{
    return p;
}

//centroidlink::t_centroids kmeans::getCentroids()
//{
//    int nDimensions;
//    
//    if (p.nPatterns() == 0)
